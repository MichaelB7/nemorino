...
template<ThreadType T> Value Search::QSearch(Value alpha, Value beta, Position& pos, int depth, ThreadData& tlData) {
	...
	if (T != ThreadType::SINGLE) ttPointer->update<tt::THREAD_SAFE>(pos.GetHash(), bestScore, nt, depth, bestMove, standPat);
	else ttPointer->update<tt::UNSAFE>(pos.GetHash(), bestScore, nt, depth, bestMove, standPat);
	return SCORE_EXACT(bestScore);
}